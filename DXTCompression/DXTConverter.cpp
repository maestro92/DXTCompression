...
// hence a 8:1 compression ratio (6:1 if original pixel are 24 bits)
...
void DXTConverter::decompressDXT3(const uint8* compressedImagePixels, uint8* outputImagePixels, int width, int height)
{
	...
	for (int y = 0; y < numBlocksY; y++)
	{
		for (int x = 0; x < numBlocksX; x++)
		{
			int startIndex = (y * numBlocksX + x) * numBytesInBlock;
			memcpy(compressedBlock, &compressedImagePixels[startIndex], numBytesInBlock);
			decompressDXT3Block(compressedBlock, outputImagePixels, x, y, width, height);
		}
	}
}


void DXTConverter::decompressDXT3Block(uint8* DXTBlock, uint8* outputImagePixels, int bx, int by, int width, int height)
{
	// we are getting four points
	uint8 colorPalette[4][4];
	...
	recreateBlockInImageDXT3(colorPalette, indices,
		alphaValues, bx, by, width, height, outputImagePixels);
}
...
void DXTConverter::recreateBlockInImageDXT3(uint8 colorPalette[][4], unsigned int* indices, 
											uint8* alphaValues, int bx, int by, int width, int height, uint8* outputImagePixels)
{
	int numBlocksX = width / 4;
	int numBlocksY = height / 4;
	uint8 color[4];
